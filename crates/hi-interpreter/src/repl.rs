...
pub fn repl_run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        if trimmed == ":vars" {
            ...
            for (sym, binding) in env_ref.bindings() {
                match binding {
                    Binding::Variable(val) => println!("  {} = {}", sym, val),
                    Binding::UserFunction(params, _) => {
                        let params_str = params
                            .iter()
                            .map(|s| s.to_string())
                            .collect::<Vec<_>>()
                            .join(", ");
                        println!("  FUNC {}({})", sym, params_str);
                    }
                    Binding::BuiltinFunction(_) => {
                        println!("  BUILTIN {}", sym);
                    }
                    Binding::Module(_) => {
                        println!("  MODULE {}", sym);
