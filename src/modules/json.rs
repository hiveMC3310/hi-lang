...
/// Creates a new built-in module for JSON serialization/deserialization.
pub fn new_json_module() -> BuiltinModule {
    let vars = HashMap::new();
    let mut funcs: HashMap<String, BuiltinFn> = HashMap::new();

    funcs.insert("parse".to_string(), Rc::new(parse_fn));
    funcs.insert("stringify".to_string(), Rc::new(stringify_fn));
...
fn hi_to_json(hi_val: &Value, span: &Span) -> InterpResult<JsonValue> {
    match hi_val {
        ...
        Value::Float(f) => {
            if f.is_finite() {
                Ok(
                    JsonValue::Number(serde_json::Number::from_f64(*f).ok_or_else(|| InterpError::Runtime {
                        span: *span,
...
/// json:parse(string) – parses a JSON string and returns a Hi value.
...
/// json:stringify(value) – converts a Hi value to a JSON string.
