...
impl<'a> Parser<'a> {
    ...
    fn consume_ident(&mut self) -> ParseResult<(String, Span)> {
        let tok = self.peek().clone();
        match &tok.kind {
    ...
    fn parse_func(&mut self) -> ParseResult<Stmt> {
        ...
        self.consume(TokenKind::End)?;
        let span = start.merge(&self.tokens[self.pos - 1].span);
        Ok(Stmt::Func(name, params, block, span))
    }
...
mod tests {
    ...
    pub fn strip_spans_stmt(stmt: &Stmt) -> Stmt {
        match stmt {
            ...
            Stmt::Func(name, params, block, _) => Stmt::Func(
                name.clone(),
                params.clone(),
                block.iter().map(strip_spans_stmt).collect(),
                Span::dummy(),
    ...
    fn test_literals() {
        ...
        assert_eq!(expr, Expr::Variable("x".to_string(), Span::dummy()));
    }
    ...
    fn test_binary_ops() {
        ...
        assert_eq!(
            ...
            Expr::Binary(
                ...
                Box::new(Expr::Binary(
                    BinOp::Eq,
                    Box::new(Expr::Variable("a".to_string(), Span::dummy())),
                    Box::new(Expr::Variable("b".to_string(), Span::dummy())),
                    Span::dummy()
                )),
                Box::new(Expr::Binary(
                    BinOp::Ne,
                    Box::new(Expr::Variable("c".to_string(), Span::dummy())),
                    Box::new(Expr::Variable("d".to_string(), Span::dummy())),
                    Span::dummy()
                )),
                Span::dummy()
            )
    ...
    fn test_unary_not() {
        ...
        assert_eq!(
            ...
            Expr::Unary(
                UnOp::Not,
                Box::new(Expr::Variable("x".to_string(), Span::dummy())),
                Span::dummy()
            )
        );
    ...
    fn test_call() {
        ...
        assert_eq!(expr, Expr::Call("bar".to_string(), vec![], Span::dummy()));
    }
    ...
    fn test_index() {
        ...
        assert_eq!(
            expr,
            Expr::Index(
                Box::new(Expr::Variable("arr".to_string(), Span::dummy())),
                Box::new(Expr::Binary(
                    BinOp::Add,
                    Box::new(Expr::Variable("i".to_string(), Span::dummy())),
                    Box::new(Expr::Int(1, Span::dummy())),
                    Span::dummy()
                )),
                Span::dummy()
        ...
        assert_eq!(
            ...
            Expr::Index(
                Box::new(Expr::Index(
                    Box::new(Expr::Variable("matrix".to_string(), Span::dummy())),
                    Box::new(Expr::Variable("i".to_string(), Span::dummy())),
                    Span::dummy()
                )),
                Box::new(Expr::Variable("j".to_string(), Span::dummy())),
                Span::dummy()
            )
        );
    }
    ...
    fn test_dict() {
        ...
        assert_eq!(
            ...
            Expr::Dict(
                vec![
                    (
                        Expr::Variable("a".to_string(), Span::dummy()),
                        Expr::Int(1, Span::dummy())
                    ),
                    (
                        Expr::Variable("b".to_string(), Span::dummy()),
                        Expr::Binary(
                            BinOp::Add,
                            Box::new(Expr::Int(2, Span::dummy())),
                            Box::new(Expr::Int(3, Span::dummy())),
            ...
            Stmt::While(cond, block, _) => {
                ...
                match &block[0] {
                    Stmt::Assign(lhs, rhs, _) => {
                        assert_eq!(**lhs, Expr::Variable("x".to_string(), Span::dummy()));
                        assert_eq!(
                            **rhs,
                            Expr::Binary(
                                BinOp::Add,
